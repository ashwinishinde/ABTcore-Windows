...
class data(xmlrpc.XMLRPC):
	...
	+ if you have ABTcore running om your machine then locate it at ``/ABT/abt/``
	"""
	def __init__(self):
	...
	def xmlrpc_getStateNames(self):
		"""
		* Purpose:
			- This is function to get state list present in the "/ABT/abt/places.db"
		* Output: 
			- return list of all the distinct states present in "/ABT/abt/places.db"
		"""
		try:
			conn = sqlite.connect("/ABT/abt/places.db")
			cur = conn.cursor()
	...
	def xmlrpc_getCityNames(self,queryParams):
		"""
		* Purpose:
			- This is function to get city list present in the "/ABT/abt/places.db"
		...
		* Output: 
			- return list of all the city names present in "/ABT/abt/places.db"
		"""
		try:
			conn = sqlite.connect("/ABT/abt/places.db")
			cur = conn.cursor()
